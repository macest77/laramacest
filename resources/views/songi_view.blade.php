...
<script>
    ...
    function checkAndSubmit() {
        ...
        if (EmailRegex.test($("#temp_songi_list_mail").val()) ) {
            var dataTxt = '';
            var datta = {};
            datta["temp_songi_list_mail"] = $("#temp_songi_list_mail").val();
            dataTxt = dataTxt + '"temp_songi_list_mail":'+ $("#temp_songi_list_mail").val();
            datta["_token"] = $("[name='_token']").val();
            datta["_token"] = $("[name='_token']").val();
            $(votes.votes).each(function(index, element) {
                votesCount = votesCount + element;
                datta['temp_songi_list_id_'+(index+1)] = element;
                dataTxt = dataTxt + ',"temp_songi_list_id_' + (index+1) + '":' + element;
            });
            ...
            if (votesCount > 0) {
                ...
                $.ajax({
                    ...
                    success:function(response){
                        $("#errors").html('<strong>' + response + '</strong>');
                    }
                    
                });
                //$.post('/lista-hard-n-heavy', {data: data}, function(returned) {
...
@section('content')
      ...
      <div id="errors"></div>
      <p>Sugestie dot. nowości wysyłaj na adres: lista@marcinstefanski.pl</p>
      <form>@csrf</form>
